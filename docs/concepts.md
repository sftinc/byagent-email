...
signed with the inbox's key and `LINK_KEY`, a secret the Worker keeps only for this, and **expires
after fifteen minutes**, because links land in transcripts and logs that outlive them. An expired link answers `410` and says to read the message again; a
...
same. Rotating `LINK_KEY` revokes every outstanding link, for every inbox; rotating the admin key
leaves links alone. Message lists carry attachment names, types and sizes but no links — read the
message to get those. Webhook payloads carry them too.
